...
class ApiService {
  ...
  static Future<TableModel> createTable(Map<String, dynamic> tableData) async {
    final url = Uri.parse('$baseUrl/tables');
    // Build payload using camelCase keys for the Node.js backend
    final payload = {
      'tableNumber': tableData['table_number'] ?? 0,
      'tableName': tableData['table_name'] ?? '',
      'totalSeats': tableData['total_seats'] ?? 0,
      'status': tableData['status'] ?? 'Active',
      'floorName': tableData['floor_name'] ?? 'Main Floor',
  ...
  static Future<void> updateTable(TableModel table) async {
    ...
    final payload = {
      'tableNumber': int.tryParse(table.number) ?? table.number,
      'tableName': table.name,
      'totalSeats': table.maxGuests,
  ...
  static Future<List<SeatModel>> getSeatsByTable(String tableId) async {
    final url = Uri.parse('$baseUrl/seats/table/$tableId');
    print('GET $url');
    try {
      final response = await http.get(url).timeout(const Duration(seconds: 10));
  ...
  static Future<void> deleteSeat(String seatId) async {
    final url = Uri.parse('$baseUrl/seats/$seatId');
    print('DELETE $url');
    try {
      final response = await http.delete(url).timeout(const Duration(seconds: 10));
  ...
  static Future<void> updateSeatBillingStatus(String seatId, bool billingStatus) async {
  ...
  static Future<Product> createProduct(Product product, {XFile? imageFile}) async {
    ...
    if (imageFile != null) {
      ...
      final contentType = MediaType.parse('image/jpeg');
      request.files.add(
  ...
  static Future<void> updateBill(String billId, List<Map<String, dynamic>> items, double total) async {
  ...
  static Future<void> confirmBill(String billId, {String? paymentMethod}) async {
